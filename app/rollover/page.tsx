...
export default async function RolloverPage() {
  ...
  return (
    <div className="min-h-screen p-6 max-w-xl mx-auto">
      ...
      <h1 className="text-xl font-semibold text-foreground mt-6 mb-2">Year Rollover</h1>
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-6">
        <h2 className="text-sm font-medium text-muted uppercase tracking-wide mb-3">Current data</h2>
        <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <dt className="text-muted">Groups</dt>
          <dd className="text-foreground font-medium">{summary.groupCount}</dd>
          <dt className="text-muted">Students</dt>
          <dd className="text-foreground font-medium">{summary.studentCount}</dd>
          <dt className="text-muted">Books</dt>
          <dd className="text-foreground font-medium">{summary.bookCount}</dd>
          <dt className="text-muted">Payment records</dt>
          <dd className="text-foreground font-medium">{summary.paymentCount}</dd>
          <dt className="text-muted">Total balance</dt>
          <dd className={`font-medium ${summary.totalBalanceYen >= 0 ? 'text-green-600' : 'text-red-500'}`}>
