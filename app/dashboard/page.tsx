...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 max-w-screen-2xl mx-auto">
      ...
      {/* Data-dependent content streams in; the header above flushes first. */}
...
async function DashboardData({ searchParams }: DashboardPageProps) {
  ...
  const groupIds = groups.map((g: ClassGroup) => g.id);

  // Fetch every group's books and students in two constant queries (instead of
  // two queries per group). student_books are pulled via a relational select,
  // then everything is bucketed by class_group_id in memory below.
  ...
  const booksByGroup = new Map<string, Book[]>();
  for (const book of (booksResult.data ?? []) as Book[]) {
    const list = booksByGroup.get(book.class_group_id);
    if (list) list.push(book);
    else booksByGroup.set(book.class_group_id, [book]);
  }
  ...
  for (const s of rawStudents) {
    ...
    const list = studentsByGroup.get(s.class_group_id);
    if (list) list.push(row);
    else studentsByGroup.set(s.class_group_id, [row]);
  }

  const allGroupData: GroupData[] = groups.map((group: ClassGroup) => ({
    group,
    books: booksByGroup.get(group.id) ?? [],
